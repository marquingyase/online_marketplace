...
export default function Header() {
    ...
    return (
        <header className="bg-white sticky top-0 z-50">
            ...
            <div className="py-4 border-b border-gray-200">
                <Container cn="flex justify-between items-center lg:gap-10 xl:gap-20">
                    ...
                    <nav className="flex items-center space-x-4 font-medium">
                        ...
                        {/* hamburger */}
                        <button onClick={() => setToggle(!toggled)} className="cursor-pointer text-gray-700 hover:text-blue-500 focus:outline-none">
                            {toggled ? <RiCloseFill className="text-3xl" /> : <CgMenuRight className="text-3xl" />}
                        </button>
                    </nav>
            ...
            {/* Navbar links */}
            <div className={`${toggled ? "block" : "hidden"} overflow-y-scroll absolute bg-slate-50 border-b border-b-gray-300 p-8 w-full`}>
                <Container>

                    {/* categories */}
                    <div className="uppercase font-semibold text-white text-sm bg-blue-500 p-2 mb-4">Categories</div>
                    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 text-sm gap-6 mb-4">
                        {categories.map((category) => (
                            <div key={category.name}>
                                <h3 className="font-bold mb-1">{category.name}</h3>
                                <ul>
                                    {category.items.map((item) => (
                                        <li key={item.name} className="flex w-fit items-center hover:text-blue-500">
                    ...
                    {/* account */}
                    <div className="uppercase md:hidden font-semibold text-white text-sm bg-blue-500 p-2 mb-4">Account</div>
                    <div className="flex justify-between md:hidden text-gray-700 mb-4">
                        ...
                        {/* cart */}
                        <Link href="/cart" className="flex items-center space-x-1 text-gray-700 hover:text-blue-500">
                            <BsCart3 className="text-2xl" />
                            <span className="text-sm">Cart</span>
                        </Link>
                    </div>
                    ...
                    {/* search */}
                    <div className="flex lg:hidden flex-1 items-center border border-blue-300 p-1 focus-within:border-blue-500">
                        <input
                            type="search"
                            placeholder="Search"
                            name="search"
